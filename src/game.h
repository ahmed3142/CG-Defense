...
class Game
{
    ...
    int getDefaultLevelIndex() const;
    const char *getDifficultyLabel() const;
    Color getDifficultyColor() const;
    int getEnemyReward(EnemyType type) const;
    ...
    GameUIState difficultyReturnState = GameUIState::MainMenu;
    ...
    // ── Feature: 2x speed toggle ─────────────────────────────────────────────
    bool gameSpeed2x = false;
    Rectangle speedBtn = {1340, 808, 140, 38};

    // ── Feature: per-round kill / earnings stats ──────────────────────────────
    int roundKills        = 0;
    int roundMoneyEarned  = 0;
    bool showRoundStats   = false;
    int lastRoundKills    = 0;
    int lastRoundMoneyEarned = 0;
    Timer roundStatsTimer = Timer(5.0f);

    // ── Feature: wave preview ─────────────────────────────────────────────────
    struct WaveComposition { int basic, fast, tank, physics, boss; };
    WaveComposition computeWaveComposition(int roundNum) const;
    void drawWavePreview();

    // ── Feature: high scores (best round reached per level) ───────────────────
    static const int MAX_SCORE_LEVELS = 50;
    int highScores[MAX_SCORE_LEVELS] = {};
    bool scoreSaved = false;
    void loadHighScores();
    void saveHighScores();

    // ── Feature: sell / upgrade buttons on selected-tower panel ──────────────
    Rectangle towerUpgradeBtnRect = {};
    Rectangle towerSellBtnRect    = {};
    ...
    Rectangle easyDifficultyBtn = {530, 280, 440, 70};
    Rectangle mediumDifficultyBtn = {530, 380, 440, 70};
    Rectangle hardDifficultyBtn = {530, 480, 440, 70};
    ...
    // main menu animation
    static const int MENU_FRAME_COUNT = 900;
    Texture2D mainMenuBackground[MENU_FRAME_COUNT] = {};  // zero-init; id==0 means not loaded yet
