...
{
    // Pick a tint colour per enemy type so each type is visually distinct
    Color tint;
    switch (type) {
        case EnemyType::basic:    tint = WHITE;                        break;
        case EnemyType::fast:     tint = {255, 240, 80,  255};         break; // yellow
        case EnemyType::tank:     tint = {220, 80,  80,  255};         break; // red
        case EnemyType::physics:  tint = {80,  220, 220, 255};         break; // cyan
        case EnemyType::finalBoss:tint = {200, 50,  255, 255};         break; // purple
        default:                  tint = WHITE;                        break;
    }

    float drawX = (position.x - size / 2) * tileSize;
    float drawY = (position.y - size / 2) * tileSize;
    float drawSize = size * tileSize;
    ...
    if (currentHealth < maxHealth) {
        ...
        float barH = 4.0f;
        float barX = drawX;
        float barY = drawY - barH - 2.0f;
        float pct   = (float)currentHealth / (float)maxHealth;

        // background
        DrawRectangle((int)barX, (int)barY, (int)barW, (int)barH, Fade(BLACK, 0.6f));
        // fill — green→yellow→red by health %
        Color fillCol = pct > 0.6f ? GREEN : (pct > 0.3f ? YELLOW : RED);
        DrawRectangle((int)barX, (int)barY, (int)(barW * pct), (int)barH, fillCol);
    }
}
